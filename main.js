...
const { truthordareCommand } = require('./commands/truthordare');
...
global.channelLink = "https://whatsapp.com/channel/0029Vb77pP4A89Mje20udJ32";
global.ytch = "C.O TECH";
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (isBanned(senderId) && !userMessage.startsWith('.unban')) {
            ...
            if (Math.random() < 0.1) {
                await sock.sendMessage(chatId, {
                    text: '❌ You are banned from using the bot. Contact an admin to get unbanned or just go and sleep.',
                    ...channelInfo
                });
            }
            return;
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (!isBotAdmin) {
                await sock.sendMessage(chatId, { text: 'make the bot an admin to use admin commands baka! .', ...channelInfo }, { quoted: message });
                return;
            }
        ...
        if (isOwnerCommand) {
            if (!message.key.fromMe && !senderIsSudo) {
                await sock.sendMessage(chatId, { text: 'This command is only available for the owner or sudo!  so just rest..' }, { quoted: message });
                return;
            }
        }
        ...
        switch (true) {
            case userMessage === '.simage': {
                ...
                if (quotedMessage?.stickerMessage) {
                    ...
                    await sock.sendMessage(chatId, { text: 'reply to a sticker with the .simage command to convert it.', ...channelInfo }, { quoted: message });
                }
            ...
            case userMessage.startsWith('.mute'):
                {
                    ...
                    if (muteArg !== undefined && (isNaN(muteDuration) || muteDuration <= 0)) {
                        await sock.sendMessage(chatId, { text: 'provide a valid number of minutes or use .mute with no number to mute immediately.', ...channelInfo }, { quoted: message });
                    } else {
...
case userMessage.startsWith('.xgroup'):
    ...
    break;
            ...
            case userMessage.startsWith('.mode'):
                ...
                if (!message.key.fromMe && !senderIsSudo) {
                    await sock.sendMessage(chatId, { text: 'Only bot owner can use this command! baka', ...channelInfo }, { quoted: message });
                    return;
                }
            ...
            case userMessage === '.tagall':
                if (isSenderAdmin || message.key.fromMe) {
                    ...
                    await sock.sendMessage(chatId, { text: 'only group admins can use the .tagall command. go and sleep peasant', ...channelInfo }, { quoted: message });
                }
                break;
        ...
        if (command === '.spam') {
            ...
            case userMessage.startsWith('.weather'):
                ...
                if (city) {
                    ...
                    await sock.sendMessage(chatId, { text: ' specify a city, e.g., .weather London', ...channelInfo }, { quoted: message });
                }
                break;
    ...
    break;
            ...
            case userMessage.startsWith('.move'):
                ...
                if (isNaN(position)) {
                    await sock.sendMessage(chatId, { text: ' provide a valid position number for Tic-Tac-Toe move.', ...channelInfo }, { quoted: message });
                } else {
    ...
    break;
            ...
            case userMessage.startsWith('.guess'):
                ...
                if (guessedLetter) {
                    ...
                    sock.sendMessage(chatId, { text: 'hmm guess a letter using .guess <letter>', ...channelInfo }, { quoted: message });
                }
                break;
            ...
            case userMessage.startsWith('.answer'):
                ...
                if (answer) {
                    ...
                    sock.sendMessage(chatId, { text: ' provide an answer using .answer <answer>', ...channelInfo }, { quoted: message });
                }
                break;
    ...
    break;
            ...
            case userMessage.startsWith('.welcome'):
                if (isGroup) {
                    ...
                    if (isSenderAdmin || message.key.fromMe) {
                        ...
                        await sock.sendMessage(chatId, { text: 'only group admins can use this command. go and sleep peasant', ...channelInfo }, { quoted: message });
                    }
                } else {
            ...
            case userMessage.startsWith('.antibadword'):
                ...
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, { text: '*make the bot admin first peasant*', ...channelInfo }, { quoted: message });
                    return;
                }
            ...
            case userMessage.startsWith('.chatbot'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: 'This command can only be used in groups. dont be stupid!', ...channelInfo }, { quoted: message });
                    return;
                }
                ...
                if (!chatbotAdminStatus.isSenderAdmin && !message.key.fromMe) {
                    await sock.sendMessage(chatId, { text: '*only admins or bot owner can use this command. so go and sleep peasant*', ...channelInfo }, { quoted: message });
                    return;
                }
    ...
    break;
            ...
            case userMessage.startsWith('.tiktok') || userMessage.startsWith('.tt'):
                ...
                break;
            case userMessage.startsWith('.gpt5') || userMessage.startsWith('.gemini'):
...
async function handleGroupParticipantUpdate(sock, update) {
    try {
        ...
        if (action === 'add') {
            ...
            const groupDesc = groupMetadata.desc || 'No description available, put a desciption for your group!';

            // Use simple default welcome message
            const welcomeMessage = '👋 Hey there {user} to {group}! so glad you’re here! 🎉 /🤗 Check out the group info below, then say hi to the rest of the members!';
